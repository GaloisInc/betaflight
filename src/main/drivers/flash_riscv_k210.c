...
static riscv_k210_w25q_t
flash_receive_data_enhanced (uint32_t *cmd_buff, uint8_t cmd_len, uint8_t *rx_buff, uint32_t rx_len)
{
	spi_receive_data_multiple (spi_bus_no, spi_chip_select, cmd_buff, cmd_len, rx_buff, rx_len);
	return FLASH_OK;
}
static riscv_k210_w25q_t
flash_send_data_enhanced (uint32_t *cmd_buff, uint8_t cmd_len, uint8_t *tx_buff, uint32_t tx_len)
{
...
static riscv_k210_w25q_t flash_quad_page_program (uint32_t addr, uint8_t *data_buf, uint32_t length)
{
	...
	spi_init_non_standard (spi_bus_no,
	                       8/*instrction length*/,
	                       24/*address length*/,
	                       0/*wait cycles*/,
